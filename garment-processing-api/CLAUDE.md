...
├── scripts/                     # Model download/conversion utilities
├── tests/                       # Runtime validation scripts
├── notebooks/                   # Training and evaluation notebooks
├── docs/                        # API, deployment, architecture, and changes
├── legacy/                      # Preserved previous app implementation
...
**Training Notebook:**
- `notebooks/train_clothing_classifier_colab.ipynb` - Full training pipeline for Google Colab
- Trains 3-class CNN (trousers, tshirt, other)
- Generates `best_clothing_model.h5`

**Evaluation Notebook (UPDATED):**
- `notebooks/evaluate_model_thesis_colab.ipynb` - Generates thesis evaluation materials
...
**Python Version:** 3.10+ (managed with uv)

### Deployment

**Railway Deployment:**
- Configuration: `nixpacks.toml`
- Automatic model download from Google Drive via `scripts/download_models_railway_v2.sh`
...
2. TensorFlow package assertion (`tensorflow-cpu` on Railway Linux)
3. Model download from Google Drive (~1-2 min)
4. Model validation with TensorFlow
5. Application startup through `uv run gunicorn`
