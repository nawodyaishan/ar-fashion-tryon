...
# File ID should be in scripts/download_models_railway.sh or scripts/download_models_railway_v2.sh
grep "FILE_ID=" scripts/download_models_railway*.sh
```
...
- TensorFlow package verification: `tensorflow-cpu` on Railway Linux, `tensorflow` on local macOS arm64
...
ls -l scripts/download_models_railway_v2.sh

# Verify FILE_ID is set
grep "FILE_ID=" scripts/download_models_railway_v2.sh
```
...
4. Update `scripts/download_models_railway_v2.sh`:
   ```bash
   FILE_ID="YOUR_NEW_FILE_ID"
   ```

**c) Increase Timeout:**
Edit `scripts/download_models_railway_v2.sh`:
```bash
...
**Verify:**
```bash
grep "unzip -o" scripts/download_models_railway_v2.sh
...
**Root Cause:** Dependencies were not synced before the start command.

**Solution:** Railway must run `uv sync --frozen --no-dev` during install and use `uv run` for scripts and startup.
...
- [ ] Run `uv run python tests/test_model_load.py` locally
- [ ] Verify Google Drive file is accessible
- [ ] Check environment variables are set in Railway
- [ ] Review recent commits for breaking changes
- [ ] Test locally with `uv run uvicorn app:app --reload`
